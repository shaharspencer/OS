...
void worker(void *arg) {
    ...
    if (tc->threadID == 0) {
        ...
        uint64_t stage = ((tc->atomicCounter)->load() >> 62 & (0x3ULL)); // TODO rm
        std::cout<<"stage at beggining of thread runs: " << stage << std::endl; // TODO rm
        tc->barrier->barrier();
        /* initialize number of keys to process */
        uint64_t keysNum = tc->inputVec->size();
        (*(tc->atomicCounter)) += keysNum << 31;

        std::cout<<"keysNum " <<keysNum <<std::endl;
        tc->barrier->barrier();
        stage = tc->atomicCounter->load() >> 31 & (0x7fffffff);
        std::cout<< "in counter: "<<stage<<std::endl;
        tc->barrier->barrier();
    }
...
void getJobState(JobHandle job, JobState *state) {
    ...
    *state = {stage_t(stage), static_cast<float>(processed * 1.0 / total)};
}
